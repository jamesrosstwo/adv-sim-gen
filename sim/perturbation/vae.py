...
class VAEFramePerturbation(Perturbation):
    def __init__(self, state_path: Path, z_size: int = 32, perturbation_strength=0.5, *args, **kwargs):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        delta = torch.nn.functional.normalize(perturbation, dim=0) * self._perturbation_strength
        delta *= mask
        return x + delta
...
class VAELatentPerturbation(Perturbation):
    ...
    GRAD_ACCUM_STEPS = 64
    N_ITERS = 100
    def __init__(self, state_path: Path, z_size: int = 32, perturbation_strength=0.01, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._vae = ConvVAE(z_size=z_size)
        vae_state_dict = torch.load(str(state_path))
        self._vae.load_state_dict(vae_state_dict, strict=True)
